...
def maestroEnvList() {
    ...
    def mhLower = maestroHome.toLowerCase()
    if (mhLower.contains('users\\hp\\maestro') || mhLower.contains('users/hp/maestro') || mhLower.contains('\\maestro\\maestro\\bin')) {
        echo "[WARN] Overriding stale MAESTRO_HOME=${maestroHome} -> ${parallelHome}"
...
pipeline {
    ...
    post {
        always {
            ...
            // Jenkins Stop often kills python but leaves maestro.cli.AppKt java.exe orphans —
            // always re-enter the devices agent and kill Maestro automation.
            script {
                try {
                    node(params.DEVICES_AGENT) {
                        catchError(buildResult: 'SUCCESS', stageResult: 'SUCCESS') {
                            if (fileExists('scripts/jenkins_abort_maestro_cleanup.bat')) {
                                ...
                                bat '''
powershell -NoProfile -NonInteractive -Command "Get-CimInstance Win32_Process -Filter \"Name='java.exe'\" | Where-Object { $_.CommandLine -match 'maestro\\.cli\\.AppKt' } | ForEach-Object { Write-Host ('[abort-cleanup] kill java ' + $_.ProcessId); taskkill /PID $_.ProcessId /T /F | Out-Null }; Get-CimInstance Win32_Process -Filter \"Name='cmd.exe'\" | Where-Object { $_.CommandLine -match 'run_one_flow_on_device\\.bat|maestro\\.bat' } | ForEach-Object { taskkill /PID $_.ProcessId /T /F | Out-Null }"
...
'''
                    ...
                    echo "[WARN] Maestro abort cleanup skipped: ${e}"
                }
            }
        }
    }
