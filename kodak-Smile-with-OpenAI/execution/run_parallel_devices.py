...
# Windows: avoid UnicodeEncodeError (e.g. AI text with Unicode hyphens) on Jenkins consoles.
if os.name == "nt":
    try:
...
from execution.archive_run_artifacts import archive_post_run_artifacts  # noqa: E402
...
def run_flow_for_device(
    ...
    *,
    ...
    use_ai: bool,
    ai_semaphore: threading.Semaphore,
...
def run_all_flows_parallel(
    ...
    *,
    ...
    for flow_path in flows:
        _flow_name = suite_and_flow(flow_path, repo)[1]
        logger.info("Flow wave (parallel across devices): %s", flow_path.name)
        for d in devices:
            _orch_print(f"[INFO] Starting {_flow_name} on {d}")
...
def legacy_device_worker(
    ...
    *,
    ...
    for flow_path in flows:
        _fn = suite_and_flow(flow_path, repo)[1]
        _orch_print(f"[INFO] Starting {_fn} on {device_id}")
        run_flow_for_device(
            device_id,
            device_name,
            flow_path,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--no-archive",
        action="store_true",
        help="Do not copy logs/reports/Excel into archive/ after the run.",
    )
    p.add_argument(
        "--ai-concurrency",
        type=int,
        default=None,
        metavar="N",
        help="Max concurrent OpenRouter AI calls (default: 2, or ORCH_AI_CONCURRENCY).",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    if args.ai_concurrency is not None:
        ai_n = max(1, min(32, int(args.ai_concurrency)))
    else:
        try:
            ai_n = int((os.environ.get("ORCH_AI_CONCURRENCY") or "2").strip() or "2")
        except ValueError:
            ai_n = 2
        ai_n = max(1, min(32, ai_n))
    ai_semaphore = threading.Semaphore(ai_n)
    logger.info("AI concurrency (OpenRouter): %s concurrent request(s)", ai_n)
    ...
    if not args.no_archive and not _truthy_env("ORCH_NO_ARCHIVE"):
        try:
            archive_post_run_artifacts(repo, excel_path)
        except OSError as e:
